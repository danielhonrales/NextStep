...
import { TextInput, View, KeyboardAvoidingView, ScrollView, TouchableOpacity, Image, StyleSheet } from "react-native"
...
export function ChatPage({navigation}) {
    ...
    async function sendAutomatedResponse(newFeed) {
        await delay(2000)
        const newerFeed = newFeed.concat(<Message user={otherUser} text={automatedResponse}  key={otherUser + getMessageTimestamp()}/>)
    ...
    return (
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={64} keyboardShouldPersistTaps='handled' style={styles.container}>
            <ScrollView  ref={ref => {this.scrollView = ref}} onContentSizeChange={() => this.scrollView.scrollToEnd({animated: true})} contentContainerStyle={styles.feed}>
                ...
                <Message user={otherUser} text="Hello!"/>
                {feed}
            </ScrollView>
